...
nf=10;
type=1;
fprintf('resdual$ dim       $ & ||f(x)|| & ||df(x)|| & time & iteration & begin subspace &\n');
      ...
      xs=xkR;
         ...
         rkR2(rkR2<0)=0;
         gR2=norm(A'*rkR2);
         fprintf('resdual$ %d \\times %d $ & %g & %g & %4.5f & %d & %d & %g,%g&\n',m,n,gR2,gR,tfR,countFR,countNWR,sum(rkR>0),sum(rkR>=0));
         ...
         rkA(rkA<0)=0;
          dA=norm(rkA);
         gA=norm(A'*rkA);
         fprintf('ALS$ %d \\times %d $ & %g & %g & %4.5f & %d & %d & %d& %g,%g\n',m,n,dA,gA,tfA,countFA,countNA,Arr(1,end),sum(rkA>0),sum(rkA>=0));
%         Arr(1,end)
...
%          fprintf('pina$ %d \\times %d $ & %g & %g & %4.2f & %d & %d &\n',m,n,dpa,gpa,tfpa,countFMpa,countNWpa);
         ...
         dhan=norm(rkhan);
         ghan=norm(A'*rkhan);
         fprintf('han$ %d \\times %d $ & %g & %g & %4.5f & %d & %d & %g,%g&\n',m,n,dhan,ghan,tfhan,countFMhan,countNWhan,sum(rkhan>0),sum(rkhan>=0));
         ...
         rkLei(rkLei<0)=0;
          dLei=norm(rkLei);
         gLei=norm(A'*rkLei);
         fprintf('Lei$ %d \\times %d $ & %g & %g & %4.5f & %d & %d & %g,%g&\n',m,n,dLei,gLei,tfLei,countFMLei,countNWLei,sum(rkLei>0),sum(rkLei>=0));
        ...
        [xkD,rkD,countFD,countND,bNWD,tfD,vkD]=Dax(x0,A,b,maxIter);
         ...
         rkD(rkD<0)=0;
          dD=norm(rkD);
         gD=norm(A'*rkD);
         fprintf('Dax$ %d \\times %d $ & %g & %g & %4.5f & %d & %d & %d& %g,%g\n',m,n,dD,gD,tfD,countFD,countND,bNWD,sum(rkD>0),sum(rkD>=0));
        ...
        [xkG,rkG,countFG,countNG,bNWG,tfG,vkG]=gradientFM_i(x0,A,b,nf,1e-8,maxIter,xs1,type);
          ...
          dG=norm(rkG);
          gG=norm(A'*rkG);
          fprintf('grad$ %d \\times %d $ & %g & %g & %4.2f & %g & %g & %g & %g,%g&\n',m,n,dG,gG,tfG,countFG,countNG,bNWG,sum(rkG>0),sum(rkG>=0));
     ...
     [xkC,rkC,countFMC,countNWC,beginNWC,tfC,vkC]=contraction_i(x0,A,b,nf,0.8,maxIter,xs1,type);
        rkC=b-A*xkC;
          ...
          dC=norm(rkC);
         gC=norm(A'*rkC);
          fprintf('con$ %d \\times %d $ & %g & %g & %4.5f & %g & %g & %g & %g,%g&\n',m,n,dC,gC,tfC,countFMC,countNWC,beginNWC,sum(rkC>0),sum(rkC>=0));
       ...
       [xkP,rkP,countFP,countNP,bNWP,tfP,vkP]=predictFM_i(x0,A,b,nf,10,maxIter,xs1,type);
        rkP=b-A*xkP;
          rkP(rkP<0)=0;
           dP=norm(rkP);
        gP=norm(A'*rkP);
         fprintf('pred$ %d \\times %d $ & %g & %g & %4.5f & %g & %g & %g & %g,%g&\n',m,n,dP,gP,tfP,countFP,countNP,bNWP,sum(rkP>0),sum(rkP>=0)); 
     ...
     fprintf('distence from han:resdual(%g),Lei(%g),Dax(%g),reduction(%g),contract(%g),predict(%g)\n',norm(xkhan-xkR),norm(xkhan-xkLei),norm(xkhan-xkD),norm(xkhan-xkG),norm(xkhan-xkC),norm(xkhan-xkP))