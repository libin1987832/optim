...
w3 = [1,-1];w4 = [1,-1];
b = 1;
y1 = (x*w1'-(b+1)*ones(n,1) < 0 && x*w3'-(b-1)*ones(n,1) > 0);
y1 = (x*w2'-(b+1)*ones(n,1) > 0 && x*w4'-(b-1)*ones(n,1) < 0);
...
% p1 = x(y,:);
% p2 = x(~y,:);
% p1= p1 + mvnrnd([0,0],[0.1,0;0,0.1],size(p1,1)); 
% p2= p2 + mvnrnd([0,0],[0.1,0;0,0.1],size(p2,1));
