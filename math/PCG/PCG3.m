...
for i=1:N
    ...
    J(3*i-2:3*i,:)=[Jtq_i -Jtq_j zeros(3,6)];
...
J'*J;
   t=tic;
detX=-(J'*J+0.001*diag(ones(1,18)))\(J'*f);
toc(t);
fxx=f;
detxx=detX;
tran3f_i=detX(1:3);
...
disp(['init error:',num2str(meam_init/(2*N)),',optimization error:',num2str(mean_opt/(2*N)),',init maxRes:',num2str(max_init),',opt maxRes:',num2str(max_opt)]);