...
% for i = 1:n
%     xw = x(i,:)*w';
%     if xw < gamm+1 && xw > gamm -1
%         index(i) = 1;
%     end
% end
%x = x(~index,:);
% y = (x*w'-b*ones(size(x,1),1) >0);
% p1 = x(y,:);
% p2 = x(~y,:);
px1 = 0:0.2:1;
p1 = [px1' -px1'+1.5];
...
[xkh2,rkh,countFMh,countNWh,beginNWh,tfh,vkh,rkArrh]=han([x0;0],A2,b2,maxIter);
norm(rkh)
ww = [w; wsvn'; xkh1';xkh1'; xkh2([1,2])' ];
bb = [b; -bsvn; gamm+1 ;gamm-1;xkh2(3)];
% 画多条线
...
%plot(d1)
[[w b]/norm(w);[wsvn' -bsvn]/norm(wsvn);[xkh1' gamm+1]/norm(xkh1);[xkh2([1,2])' xkh2(3)]/norm(xkh2([1,2]))]
